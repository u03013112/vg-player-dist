(function () {
  ...
  // ==========================================================================
  // 欲漫涩书签播放器 (观影导航 #7: mf6b8.com → 欲漫涩, 漫画为主+视频/动漫)
  // 目标站点: d1q9vvaj8cdkjj.cloudfront.net
  //
  // 平台: OIO 老代 —— 与 OIO 本体同一套签名体系:
  // - HmacSHA1 签名 key=kaFttDJRcahRMTI7 (站方 HTTP_DECRYPT_CONFIG 明文),
  //   签名算法已离线复刻并与 App 抓包逐字符验证一致 (2026-09-14)
  // - 响应解密 interfaceKey 同款 (vEukA&w15z4VAD3kAY#fkL#rBnU!WDhN)
  // - 端点族同款: /login/guest → /media/play → /media/h5/m3u8/<path>
  //
  // 与 OIO 本体的差异:
  // - X-User-Agent 短格式, Ver 固定 4.6.3:
  //   DevType=...;SysType=h5_ios;Ver=4.6.3;BuildID=<浏览器> <版本>
  //   (逆向自站方 encodeParamToHttpRequest; 服务端不校验 UA 内容,
  //   只要签名消息与请求头一致即可 —— Python 客户端已验证)
  // - login/guest 的 body sign 字段可选 (guest_skey=blPaaWHuRMiBgQzIhuV3E,
  //   sign=HmacSHA1(devID&affCode, V+guest_skey), V 来源未定) ——
  //   实测不传也曾注册成功, 故省略
  // - 视频播放页路由: /play/longVideo/<id> (路径参数) 与
  //   /play/shortVideo?videoId=<id> (bundle VideoMainPage/_id_ chunk 证据)
  //
  // 原理 (与 OIO 书签一致): 拿 token 调 /media/play 拿完整 m3u8 路径, 下载 m3u8
  // 后按 #EXT-X-KEY 声明的 URI 动态获取 16 字节 AES-128 密钥, 内联成 data URI,
  // 交给新标签页里的 hls.js。
  //
  ...
  // SysType 按设备推导 (h5_pc/h5_android/h5_ios), Ver 固定 4.6.3,
  ...
  function buildUA() {
    ...
    return 'DevType=Apple iPhone mobile;SysType=' + sys + ';Ver=4.6.3;BuildID=' + browser;
  }
  ...
  async function fetchMediaPlay(token, ua, id) {
    ...
    // preFileName 泄露(2026-09-15 哔哩同族抓包证实): 未购买时 videoUrl 可能是广告
    // 替代片, 站方自己的播放器实际加载 preFileName 指向的真实路径, 交给 resolveRecord 裁决。
  ...
  function mountInNewTab(playerWin, title, m3u8Text, realUrl) {
    ...
    try {
      ...
      alert('[vg-yms] 写入新标签页失败: ' + e.message);
    }
  }
  ...
  function loadingHtml() {
    return '<!DOCTYPE html><html><head><meta charset="utf-8">' +
      ...
      '<div style="text-align:center">VG Player YMS<br>' +
  ...
  function errorHtml(msg) {
    ...
    return '<!DOCTYPE html><html><head><meta charset="utf-8">' +
      ...
      '<div><strong style="color:#f44">VG Player YMS ❌</strong><br><br>' + safe + '<br><br>' +
  ...
  async function main() {
    ...
    try {
      ...
      console.error('[vg-yms]', e);
      alert('[vg-yms] ❌ ' + e.message);
    }
  }
