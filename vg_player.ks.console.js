  ...
  function buildPlayerHtml(rec, sourceNote) {
    ...
    return '<!DOCTYPE html><html><head><meta charset="utf-8">' +
      ...
      '<script>(function(){' +
        ...
        // lastProgressAt/lastCT: 静默卡死兜底 —— hls.js 内部自愈失败时(不报 fatal,
        // 但播放头就是不推进)不会触发 ERROR 事件,只能靠 tick() 里主动侧写检测。
        // retryCount/MAX_RETRY: fatal 错误重试上限,避免同一视频反复重试陷入死循环。
        ...
        'hls.on(Hls.Events.ERROR,function(_,d){' +
          ...
          'if(!d.fatal)return;' + // 非致命交给上面 tick() 的静默卡死兜底处理
          'retryCount++;' +
